...
void MaxPosterior::LBFGSB(IM im, popTree* poptree, Chain coldCh, unsigned int nProcs, unsigned int crr_procID)
{  
  // declare
  //  typedef computeLogJointDensity<double> TcomputeLogJointDensity;
  // typedef typename TcomputeLogJointDensity::TVector TVector;

  // create object
  computeLogJointDensity<double> f(im, poptree, coldCh, nProcs, crr_procID);

  // initialize
  computeLogJointDensity<double>::TVector paraVec = computeLogJointDensity<double>::TVector::Random(6);
...
template <> double computeLogJointDensity<double>::computeLogJointDensity_mutationScalars_MPI_overSubLoci(TVector paraVec, MaxPosterior MAPestimate, IM im, popTree* poptree, Chain coldCh, unsigned int nProcs, unsigned int crr_procID)
{
