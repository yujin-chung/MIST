...
class LbfgsbSolver : public ISolver<TProblem, 1> {
  ...
  TVector &SubspaceMin) {
	...
	////////// xing, 12/11/2017, print out vectors: V, du, dStar and scalar alpha_star in SubspaceMinimization method, all cout involves //////////////
	std::cout << "In lbfgsbsolver.SubspaceMinimization():" << endl;
	std::cout << "\t After Step 2, V = [";
	for (TIndex j = 0; j < v.rows(); j++) 
		std::cout << v[j] << "  ";
	std::cout << "]" << endl; 
...
///////////// xing, 11/29/2017, comment out these cout, the change can be seen from previous iteration //////////////
//    std::cout <<"In minize()\n";
//    std::cout <<"x = " <<x <<"\n";
//    std::cout <<"g = " <<g <<"\n";
//    std::cout <<"f = " <<f <<"\n";
	...
	////////////////// xing, 11/29/2017, change the printing format for some variables in minimize() method, including x, g, obj, and add rate and SubspaceMin //////////////
	std::cout << "In lbfgsbsolver.minimize():" << endl; 
      std::cout <<"  current x = ["; // <<x <<"\n";
	for (TIndex j = 0; j < x.rows(); j++) 
		std::cout << x[j] << "  ";
	std::cout << "]"<< endl;
      std::cout <<"  current g = ["; // <<g <<"\n";
	for (TIndex j = 0; j < g.rows(); j++) 
		std::cout << g[j] << "  ";
	std::cout << "]" << endl;
      std::cout <<"  current f = " << f <<"\n";
      std::cout <<"  rate = " << rate <<"\n";
      std::cout <<"  SubspaceMin = [" ; //<< SubspaceMin <<"\n";
	for (TIndex j = 0; j < SubspaceMin.rows(); j++) 
		std::cout << SubspaceMin[j] << "  ";
	std::cout << "]" << endl;
	std::cout << "\tCauchyPoint = ["; 
	for (TIndex j = 0; j < CauchyPoint.rows(); j++)
		std::cout << CauchyPoint[j] << "  ";
	std::cout << "]" << endl;
	std::cout << "\tc = [";
	for (TIndex j = 0; j < c.rows(); j++)
		std::cout << c[j] << "  ";
	std::cout << "]" << endl;
...
///////////// xing, 11/29/2017, comment out some cout
//      std::cout <<"new x = " <<x <<"\n";
//      std::cout <<"new g = " <<g <<"\n";
//      std::cout <<"new f = " <<f <<"\n";
/////////// xing, empty line indicates results of next iteration
	std::cout << endl;
